...
;( function($){
	...
	var hcnt = $(".page-section5 .page-section-inn").outerHeight()+110;
	var tt = rsth = hcnt - wh;
	$(".page-section5 .page-section-inn").css({"transition":"0.5s","margin-top":-tt});				
	...
	$.fn.fsvs = function( options ) {
		...
		var defaults = {
			...
			afterSlide : function(){
				var sh = $('.wrap').attr('id');
				if($("body").hasClass("active-slide-3") && sh !=1){
					$('.wrap').attr('id', '1');
					$('.counter').counterUp({
						delay: 10,
						time: 1000
			...
			beforeSlide : function(){
				$(".page-section5 .page-section-inn").css({"transition":"0.5s","margin-top":0});
				if($("body").hasClass("active-slide-6")){
					var wh = $(window).outerHeight();
					var hcnt = $(".page-section5 .page-section-inn").outerHeight()+110;
					var tt = rsth = hcnt - wh;
					$(".page-section5 .page-section-inn").css({"transition":"0.5s","margin-top":-tt});
				}
				if($("body").hasClass("active-slide-1")){
					//$('.gnb_nav ul li').addClass('active');
				}
				},
			endSlide : function(){},
		...
		var changeViaHash = function() {
			...
			if($("body").hasClass("active-slide-6")){
					$("#fsvs-body").css("transform","translate(0px, -433%)");
				};
			...
			if($("body").hasClass("active-slide-1")){
				...
				}else if($("body").hasClass("active-slide-2")){
				...
				}else if($("body").hasClass("active-slide-3")){
					scrollTop = $(window).height()*2;
					scrollPercent = (scrollTop) / (docHeight - winHeight);
					scrollPercentRounded = Math.round(scrollPercent*100);
					$('.progressbar').css('width', scrollPercentRounded +"%");
				}else if($("body").hasClass("active-slide-4")){
					scrollTop = $(window).height()*3;
					scrollPercent = (scrollTop) / (docHeight - winHeight);
					scrollPercentRounded = Math.round(scrollPercent*100);
					$('.progressbar').css('width', scrollPercentRounded +"%");
				}else if($("body").hasClass("active-slide-5")){
		...
		var cssSlide = function( index ) {
			options.beforeSlide( index );
			body.css({
				'-webkit-transform' : 'translate(0, -' + (index*100) + '%)',
				'-moz-transform' : 'translate(0, -' + (index*100) + '%)',
				'-ms-transform' : 'translate(0, -' + (index*100) + '%)',
...
//			});
		...
		var app = {
			...
			addClasses : function( before, after ) {
				var _body = $('html.fsvs').find('body');
				var fsvs_selector = 'html.fsvs #fsvs-body' + options.selector;
				_body.removeClass( removeClass = 'active-slide-' + (before+1) );
				_body.addClass( 'active-slide-' + (after+1) );
				$( fsvs_selector ).eq( before ).removeClass( 'active-slide' );
				$( fsvs_selector ).eq( after ).addClass( 'active-slide' );
